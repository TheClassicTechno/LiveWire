...
import { useSensorPointLayer } from "./SensorPointLayer";
...
const LosAngelesMap = () => {
  ...
  // Handle sensor click to open details panel
  const handleSensorClick = (sensorData) => {
    console.log('🔌 Sensor clicked:', sensorData);
    setSensorPanelOpen(true);
  };

  // Initialize sensor layer when in San Francisco
  useSensorPointLayer(map, currentCity === 'san-francisco' ? handleSensorClick : null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      map.current.on("load", () => {
        ...
        // Initialize transmission data source
  ...
  useEffect(() => {
    console.log(
      "Data update effect fired. Map exists:",
      !!map.current,
      "Source exists:",
      !!map.current?.getSource("cable-network"),
      "Data exists:",
      !!transmissionData,
      "MapLoaded:",
      mapLoaded
    ...
    if (!source) {
      console.log(
        "Cable-network source not found, waiting for map to be fully loaded"
      );
      return;
    }
    ...
    const lineCount =
      transmissionData.features?.filter(
        (f) => f.geometry?.type === "LineString"
      ).length || 0;
    const pointCount =
      transmissionData.features?.filter((f) => f.geometry?.type === "Point")
        .length || 0;
    ...
    const sampleLine = transmissionData.features?.find(
      (f) => f.geometry?.type === "LineString"
    );
    const samplePoint = transmissionData.features?.find(
  ...
  return (
    ...
    >
      ...
      <SensorDetailsPanel isOpen={sensorPanelOpen} onClose={() => setSensorPanelOpen(false)} />
            ...
            <div className="modal-content">
              ...
              <div className="risk-assessment">
                ...
                {isLoadingRisk ? (
                  ...
                  <div
                    className={`risk-result risk-${riskAssessment.level.toLowerCase()}`}
                  >
